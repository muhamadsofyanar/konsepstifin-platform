...
export default async function TestLandingPage() {
  ...
  return <div className="public-site journey-site test-landing">
    ...
    <main>
      ...
      <section id="layanan" className="section product-section"><div className="section-heading"><span>PILIHAN LAYANAN</span><h2>Pilih untuk diri sendiri, keluarga, atau kelompok Anda.</h2><p>Isi wilayah singkat, lihat kandidat promotor, lalu lanjut checkout SEJOLI. Jika masih ragu, gunakan bantuan pemilihan layanan di bawah kartu.</p></div><div className="product-grid">{publicProducts.map((product) => <article className={product.featured ? 'product-card featured-product' : 'product-card'} key={product.title}>{product.featured && <div className="product-ribbon">PALING SERING DIPILIH</div>}<small>{product.eyebrow}</small><h3>{product.title}</h3><p className="product-description">{product.description}</p><div className="product-price"><b>{product.price}</b><span>{product.priceNote}</span></div><div className="product-list"><strong>Yang didapat</strong>{product.features.map((item) => <span key={item}>✓ {item}</span>)}</div>{product.bonuses.length > 0 && <div className="bonus-box"><strong>Bonus pilihan</strong>{product.bonuses.map((item) => <span key={item}>＋ {item}</span>)}</div>}<PublicInterestAction requirePrecheckout className="product-action" linkKey={product.productKey} checkoutUrl={product.checkoutUrl} label={`${product.action} →`} service={product.title} /></article>)}</div><div className="catalog-support"><div><b>Belum yakin memilih paket?</b><span>Tim dapat membantu mencocokkan jumlah peserta, kota, dan kebutuhan Anda.</span></div><PublicInterestAction className="catalog-support-action" linkKey="previewPromotor" label="Butuh bantuan memilih? →" service="Bantuan memilih layanan STIFIn" /></div><p className="price-note">Harga mengikuti kartu produk SEJOLI dan akan terlihat kembali pada halaman checkout. Tes STIFIn bukan layanan diagnosis medis atau psikologis.</p></section>

      <ActivityGallery />

      <section id="proses" className="process-section"><div><span>PROSES TES</span><h2>Dari pemesanan sampai pembahasan, alurnya dibuat sederhana.</h2><p>Data wilayah disimpan sebelum pembayaran agar kandidat promotor dapat ditampilkan. Tim kemudian mengonfirmasi promotor final dan jadwal setelah order.</p><Link className="dark-button" href="#layanan">Pilih layanan →</Link></div><ol><li><b>01</b><div><h3>Isi wilayah & checkout</h3><p>Simpan data singkat, lihat kandidat promotor berdasarkan wilayah, lalu selesaikan pembayaran melalui SEJOLI.</p></div></li><li><b>02</b><div><h3>Konfirmasi promotor & jadwal</h3><p>Tim mencocokkan order, menetapkan promotor final, dan mengonfirmasi waktu pelaksanaan.</p></div></li><li><b>03</b><div><h3>Ikuti tes offline</h3><p>Promotor melakukan pemindaian sepuluh sidik jari menggunakan perangkat tes.</p></div></li><li><b>04</b><div><h3>Bahas hasilnya</h3><p>Anda menerima hasil dan penjelasan awal agar tahu bagian mana yang dapat mulai diterapkan.</p></div></li></ol></section>
