...
export default async function TestLandingPage() {
  ...
  return <div className="public-site journey-site test-landing">
    ...
    <main>
      ...
      <section id="layanan" className="section product-section"><div className="section-heading"><span>PILIHAN LAYANAN</span><h2>Pilih untuk diri sendiri, keluarga, atau kelompok Anda.</h2><p>Isi data singkat agar sistem dapat mencatat consent, mencocokkan promotor, lalu menampilkan jalur checkout di app.konsepstifin.com. Jika masih ragu, gunakan bantuan pemilihan layanan di bawah kartu.</p></div><div className="product-grid">{publicProducts.map((product) => <article className={product.featured ? 'product-card featured-product' : 'product-card'} key={product.title}>{product.featured && <div className="product-ribbon">MULAI DARI 1 PESERTA</div>}<small>{product.eyebrow}</small><h3>{product.title}</h3><p className="product-description">{product.description}</p><div className="product-price"><b>{product.price}</b><span>{product.priceNote}</span></div><div className="product-list"><strong>Yang didapat</strong>{product.features.map((item) => <span key={item}>✓ {item}</span>)}</div>{product.bonuses.length > 0 && <div className="bonus-box"><strong>Bonus pilihan</strong>{product.bonuses.map((item) => <span key={item}>＋ {item}</span>)}</div>}<PublicInterestAction leadType="test_service" captureLead className="product-action" linkKey={product.productKey} checkoutUrl={product.checkoutUrl} label={`${product.action} →`} service={product.title} /></article>)}</div><div className="catalog-support"><div><b>Belum yakin memilih paket?</b><span>Tim dapat membantu mencocokkan jumlah peserta, kota, dan kebutuhan Anda.</span></div><PublicInterestAction leadType="test_service" captureLead className="catalog-support-action" linkKey="tesPersonal" label="Butuh bantuan memilih? →" service="Bantuan memilih layanan STIFIn" /></div><p className="price-note">Harga bersumber dari katalog produk yang sama dengan checkout dan akan terlihat kembali sebelum pembayaran. Tes STIFIn bukan layanan diagnosis medis atau psikologis.</p></section>

      <ActivityGallery />

      <section id="proses" className="process-section"><div><span>PROSES TES</span><h2>Dari kebutuhan sampai pembahasan, setiap langkah mempunyai tujuan.</h2><p>Formulir awal mencatat kota dan persetujuan kontak. Setelah promotor cocok ditemukan, Anda dapat melanjutkan checkout layanan tes. Pemindaian dan pembahasan tetap dilakukan secara tatap muka.</p><Link className="dark-button" href="#layanan">Pilih layanan →</Link></div><ol><li><b>01</b><div><h3>Kirim kebutuhan</h3><p>Pilih layanan, isi domisili, dan berikan persetujuan kontak serta penugasan promotor.</p></div></li><li><b>02</b><div><h3>Pencocokan promotor</h3><p>Sistem mencari kandidat berdasarkan cakupan wilayah; jadwal tetap berdasarkan konfirmasi.</p></div></li><li><b>03</b><div><h3>Lanjutkan checkout</h3><p>Setelah lead tersimpan, tautan pembayaran layanan tes diarahkan ke app.konsepstifin.com.</p></div></li><li><b>04</b><div><h3>Ikuti tes dan pembahasan</h3><p>Promotor melakukan pemindaian sepuluh sidik jari lalu menjelaskan hasil awalnya.</p></div></li></ol></section>
