...
export default async function PromoterProfilePage({ params }: { params: Promise<{ 'slug-promotor': string }> }) {
  ...
  return <main className="region-page promoter-profile">
    ...
    <header className="region-hero promoter-profile-hero"><span>DATA PROMOTOR AKTIF</span><h1>{promoter.name}</h1><p>Gunakan profil ini untuk memeriksa KodeID, cabang, dan area layanan. Kontak pribadi tidak ditampilkan; jadwal baru dibahas setelah kebutuhan tes dikirim.</p><PublicInterestAction leadType="test_service" captureLead linkKey="tesPersonal" label="Ajukan kebutuhan tes →" service="Tes STIFIn Personal" className="public-cta big" provinceName={promoter.province} regencyName={promoter.area} /></header>
    ...
    <section className="region-section region-cta"><h2>Kirim kebutuhan tanpa mengekspos kontak promotor</h2><p>Formulir mencatat persetujuan Anda sebelum data kontak dibagikan kepada promotor yang ditugaskan. Jadwal berdasarkan konfirmasi.</p><PublicInterestAction leadType="test_service" captureLead linkKey="tesPersonal" label="Kirim kebutuhan tes →" service="Bantuan memilih layanan STIFIn" className="dark-button" provinceName={promoter.province} regencyName={promoter.area} /></section>
  </main>;
}
