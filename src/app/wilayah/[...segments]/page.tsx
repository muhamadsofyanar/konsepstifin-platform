...
export default async function WilayahPage({ params }: { params: Promise<{ segments: string[] }> }) {
  ...
  return <main className="region-page">
    ...
    <header className="region-hero"><span>{levelLabel(current.level).toUpperCase()}</span><h1>Periksa cakupan Tes STIFIn di {current.name}</h1><p>Lihat data promotor aktif dan informasi cakupan sebelum mengirim kebutuhan. Jadwal, lokasi pertemuan, dan penugasan tetap berdasarkan konfirmasi.</p><Link className="public-cta big" href="/tes-stifin#layanan">Bandingkan layanan tes →</Link></header>
    ...
    <section className="region-section region-cta"><h2>Sudah mengetahui layanan yang dibutuhkan?</h2><p>Pilih paket, sampaikan lokasi, lalu tunggu konfirmasi promotor dan jadwal sebelum hadir.</p><Link className="dark-button" href="/tes-stifin#layanan">Bandingkan layanan tes →</Link></section>
  </main>;
}
