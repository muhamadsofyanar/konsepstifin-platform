...
export async function generateMetadata({ params }: { params: Promise<{ segments: string[] }> }): Promise<Metadata> {
  ...
  return { title: `Tes STIFIn ${item.name} | Konsep STIFIn`, description: `Temukan informasi Tes STIFIn, layanan, dan promotor di ${item.name}.`, alternates: { canonical: wilayahChainPath(chain) } };
}
...
export default async function WilayahPage({ params }: { params: Promise<{ segments: string[] }> }) {
  ...
  return <main className="region-page">
    ...
    {promoters.length > 0 && <section className="region-section"><span>PROMOTOR TERSEDIA</span><h2>Promotor di {current.name}</h2><div className="region-promoters">{promoters.filter((promoter) => promoter.active).map((promoter) => <article key={promoter.code}><h3>{promoter.name}</h3><p>Kode promotor: {promoter.code}</p><small>{promoter.menerimaKunjungan ? 'Menerima kunjungan' : 'Jadwal berdasarkan konfirmasi'}</small></article>)}</div></section>}
