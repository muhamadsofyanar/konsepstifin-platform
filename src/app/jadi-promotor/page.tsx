...
export default async function PromoterLandingPage() {
  ...
  const mainInvestment = promoterSteps.slice(1).reduce((total, item) => total + Number(item.price.replace(/\D/g, '') || 0), 0);
  ...
  return <div className="public-site journey-site promoter-landing">
    ...
    <main>
      ...
      <section id="tahapan" className="section promoter-section"><div className="section-heading"><span>PROGRAM, MANFAAT & BIAYA</span><h2>Mulai dari mengenal profesinya, bukan langsung membeli alat.</h2><p>Harga ditempatkan di setiap tahap agar Anda dapat menyiapkan perjalanan dengan tenang. Posisi harga dipilih di tengah: tidak menekan nilai pelatihan, tetapi tetap lebih terjangkau dibanding sejumlah penawaran pasar.</p></div><div className="promoter-path">{promoterSteps.map((step, index) => <article key={step.productKey}><div className="step-top"><b>{String(index + 1).padStart(2, '0')}</b><span>{step.eyebrow}</span></div><h3>{step.title}</h3><div className="promoter-price"><b>{step.price}</b><small>{step.priceNote}</small></div><p>{step.description}</p><ul>{step.features.map((benefit) => <li key={benefit}>✓ {benefit}</li>)}</ul><PublicInterestAction linkKey={step.productKey} checkoutUrl={step.checkoutUrl} label={`${step.action} →`} service={step.title === 'Preview Promotor' ? 'Preview Calon Promotor' : step.title === 'ID & Scanner' ? 'Informasi ID & Alat' : step.title} /></article>)}</div><div className="promoter-investment"><div><span>INVESTASI TAHAP UTAMA</span><strong>{new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 }).format(mainInvestment)}</strong><small>WSL 1 + WSL 2 + ID & scanner. Tes Personal prasyarat dan biaya lain di luar rincian ini.</small></div><p>Pembelian dilakukan bertahap mengikuti kelulusan dan persyaratan, sehingga Anda tidak harus membayar seluruhnya pada hari pertama.</p></div><div className="promoter-note"><div><b>Preview</b><span>Kenali profesinya</span></div><i>→</i><div><b>WSL 1</b><span>Bangun fondasi</span></div><i>→</i><div><b>WSL 2</b><span>Pendalaman</span></div><i>→</i><div><b>ID & scanner</b><span>Aktivasi sesuai syarat</span></div></div></section>
