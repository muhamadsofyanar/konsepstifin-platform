    ...
    <main>
      ...
      {panel === 'articles' ? <>
        ...
        {showAi && <form className="ai-generator ai-simple" onSubmit={generateWithAi}>
          ...
          <section className="ai-knowledge-picker"><header><label><input type="checkbox" checked={aiForm.useKnowledge} onChange={(event) => setAiForm((current) => ({ ...current, useKnowledge: event.target.checked }))} /><span><b>4. Gunakan sumber faktual STIFIn</b><small>AI hanya mengambil fakta dari PDF yang ditandai Fakta STIFIn. Kerangka C3H, BenBi, dan FSP diterapkan otomatis untuk struktur, tanpa mengirim materi mentahnya.</small></span></label><Link href="/admin/pustaka">Kelola pustaka →</Link></header>{aiForm.useKnowledge && <>{initialKnowledgeSources.filter((source) => source.enabledForAi && source.status === 'ready' && source.purpose === 'stifin_factual').length ? <div className="ai-source-choices"><button type="button" className={!aiForm.knowledgeSourceIds.length ? 'active' : ''} onClick={() => setAiForm((current) => ({ ...current, knowledgeSourceIds: [] }))}><b>Otomatis</b><small>Pilih fakta berdasarkan topik</small></button>{initialKnowledgeSources.filter((source) => source.enabledForAi && source.status === 'ready' && source.purpose === 'stifin_factual').map((source) => { const selected = aiForm.knowledgeSourceIds.includes(source.id); return <button type="button" key={source.id} className={selected ? 'active' : ''} onClick={() => setAiForm((current) => ({ ...current, knowledgeSourceIds: selected ? current.knowledgeSourceIds.filter((id) => id !== source.id) : [...current.knowledgeSourceIds, source.id] }))}><b>{selected ? '✓ ' : ''}{source.title}</b><small>{source.category} · {source.pageCount} hlm.</small></button>; })}</div> : <div className="ai-knowledge-empty">Belum ada sumber faktual yang aktif. <Link href="/admin/pustaka">Unggah dan verifikasi PDF fakta STIFIn.</Link></div>}</>}</section>
