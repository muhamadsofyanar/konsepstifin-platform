...
import type { ArticleContentType, ArticleInput, StoredArticle } from '@/lib/article-store';
...
export default function ArticleEditor({ databaseReady, aiReady, aiProvider, aiModel, initialArticles, initialError }: { databaseReady: boolean; aiReady: boolean; aiProvider: string; aiModel: string; initialArticles: StoredArticle[]; initialError: string }) {
      ...
      {panel === 'articles' ? <>
        ...
        {showAi && <form className="ai-generator ai-simple" onSubmit={generateWithAi}>
          ...
          <div className="ai-simple-grid">
            <div className="ai-option-group"><b>4. Jumlah artikel</b><div className="ai-choice-row compact">{([1, 3, 5] as const).map((count) => <button key={count} type="button" className={aiForm.count === count ? 'active' : ''} onClick={() => setAiForm((current) => ({ ...current, count }))}>{count} artikel</button>)}</div></div>
            <div className="ai-option-group"><b>5. Simpan sebagai</b><div className="ai-choice-row compact"><button type="button" className={aiForm.resultMode === 'draft' ? 'active' : ''} onClick={() => setAiForm((current) => ({ ...current, resultMode: 'draft' }))}>Draf</button><button type="button" className={aiForm.resultMode === 'scheduled' ? 'active' : ''} onClick={() => setAiForm((current) => ({ ...current, resultMode: 'scheduled' }))}>Terjadwal</button></div></div>
          </div>
