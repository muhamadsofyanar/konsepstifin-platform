...
export default function ArticleEditor({ databaseReady, aiReady, aiProvider, aiModel, initialArticles, initialKnowledgeSources, initialError }: { databaseReady: boolean; aiReady: boolean; aiProvider: string; aiModel: string; initialArticles: StoredArticle[]; initialKnowledgeSources: KnowledgeSource[]; initialError: string }) {
  ...
  return <div className="article-admin">
    <header className="article-admin-header"><Link href="/"><Image src="/stifin-konsep-wordmark.png" alt="STIFIn Konsep" width={419} height={168} priority /></Link><nav><span>Portal Tim</span><b>Artikel & Edukasi</b></nav><div><Link href="/admin/pustaka">Pustaka STIFIn</Link><Link href="/edukasi" target="_blank">Lihat edukasi ↗</Link><button onClick={logout}>Keluar</button></div></header>
    <main>
